...
import 'package:weather_news_app/constants.dart/end_points.dart';
...
class NewsProvider with ChangeNotifier {
  ...
  final StreamController<List<dynamic>> _newsStreamController = StreamController<List<dynamic>>.broadcast();
  ...
  Future<void> fetchNews(String category, {String? weatherCondition, bool filterBasedOnWeather = false}) async {
    _loading = true;
    notifyListeners();
  ...
  Future<void> filterNewsBasedOnWeather(String weatherCondition) async {
    ...
    for (var article in _news) {
      ...
      if ((weatherCondition == 'cold') && (analysis.words.good.length < analysis.words.bad.length)) {
        filteredNews.add(article);
      } else if (weatherCondition == 'hot' &&  (analysis.words.good.length > analysis.words.bad.length)) {
        filteredNews.add(article);
      } 
    }
